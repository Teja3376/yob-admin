...
export class PinataFileManager {
  ...
  constructor() {
    if (!process.env.NEXT_PUBLIC_VITE_PINATA_JWT || !process.env.NEXT_PUBLIC_VITE_GATEWAY_URL) {
      console.error("IPFS env check:", {
        VITE_PINATA_JWT: process.env.NEXT_PUBLIC_VITE_PINATA_JWT,
        VITE_GATEWAY_URL: process.env.NEXT_PUBLIC_VITE_GATEWAY_URL,
      });
    }
    if (!process.env.NEXT_PUBLIC_VITE_PINATA_JWT) {
      throw new Error('VITE_PINATA_JWT environment variable is required');
    }
    if (!process.env.NEXT_PUBLIC_VITE_GATEWAY_URL) {
    ...
    this.gateway = process.env.NEXT_PUBLIC_VITE_GATEWAY_URL;
  }
