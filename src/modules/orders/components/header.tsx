        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-gray-500">
            ...
            <CopyIcon
              ...
              onClick={() => handleCopy(orderDetail?._id)}
            />
          </div>
          ...
          {orderDetail?.transactionHash && (
            <div className="flex items-center gap-2 text-gray-500 text-sm">
              ...
              <p
                ...
                onClick={() =>
                  handleViewOnBlockchain(orderDetail?.transactionHash)
                }
              >
                {orderDetail?.transactionHash?.slice(0, 6)}...
                {orderDetail?.transactionHash?.slice(-4)}{" "}
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            <p
              ...
              onClick={() =>
                router.push(`/issuers/${orderDetail?.issuer?._id}`)
              }
            >
              {orderDetail?.issuer?.firstName} {orderDetail?.issuer?.lastName}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <p
              className="text-primary text-sm hover:underline cursor-pointer"
              onClick={() =>
                router.push(`/asset-list/${orderDetail?.asset?._id}`)
              }
            >
              {orderDetail?.asset?.name}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <p
              className="text-primary text-sm hover:underline cursor-pointer"
              onClick={() => router.push(`/issuers/${"1234"}`)}
            >
              {orderDetail?.investor?.name}
            </p>
          </div>
        </div>
      </div>
