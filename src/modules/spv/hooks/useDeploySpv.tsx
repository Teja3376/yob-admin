...
export const useDeploySpv = () => {
    ...
    const handleDeploySpv = async(spv:any) => {
        ...
        const {_id, name, type, currency, jurisdiction, formationDate, businessPurpose, memoAndTerms, legalDocuments } = spv;
        ...
        try {
            const metaDataCID = await uploadSpvFiles({
                ...
                legalDocuments:{
                    articlesOfAssociation:{
                        articlesOfAssociation_name: legalDocuments?.articlesOfAssociation?.name||"",
                        articlesOfAssociation_url: legalDocuments?.articlesOfAssociation?.url||"",
                    },
                    llcOperatingAgreement:{
                        llcOperatingAgreement_name: legalDocuments?.llcOperatingAgreement?.name||"",
                        llcOperatingAgreement_url: legalDocuments?.llcOperatingAgreement?.url||"",
                    },
                    memorandumOfAssociation:{
                        memorandumOfAssociation_name: legalDocuments?.memorandumOfAssociation?.name||"",
                        memorandumOfAssociation_url: legalDocuments?.memorandumOfAssociation?.url||"",
                    },
                    otherDocuments:{
            ...
            const metaCIDString = ethers.utils.formatBytes32String(metaDataCID.toString());
